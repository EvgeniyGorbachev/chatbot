...
exports.webChat = (req, res, next) => {
  ...
  function attachConversationToAgent(req) {
    if (req.body.appUser['_id'] && req.body.app['_id']) {
        db.Conversations.findOne({where: {sender: req.body.appUser['_id']}})
            .then(function (conv) {
                if (conv && conv.userId == null) {
                    // Attention HARDCODE
                    return conv.update({"userId": '1'})
                } else {
            ...
            .then(function(conversation) {
                if (conversation) {
                    req.dashboardChatSocket.emit('webhook', {type: 'new conversation added', userId: req.body.appUser['_id'], appId: req.body.app['_id'], agentId: conversation.userId})
                }
            })
            .catch(next)
    }
