...
angular.module('campaignsApp.agentChat', [])
    .controller('AgentChatController', function($scope, $timeout) {
            ...
            $('#file').change(function(e) {
                ...
                if (vm.isValidFile(file) && file.type != 'image/png' && file.type != 'image/jpeg') {
                    ...
                    ss(socket).emit('file', stream, {
                        "name": file.name,
                        "userId": vm.currentUser.sender,
                        "campaign_id": vm.currentUser.campaign_id,
                    });
                    ...
                    smooch.appUsers.uploadImage(vm.currentUser.sender, file, {
                        ...
                        vm.isDownloadingFile = false;
                        socket.emit('getUserConversation', {
                            "smoochUserId": vm.currentUser.sender,
                            "campaignId": vm.currentUser.campaign_id
                        });
                    });
                }
        ...
        vm.paused = function() {
            ...
            socket.emit('pausedConversation', {
                "conversationId": vm.currentUser.id,
                "isPaused": vm.currentUser.isPaused
            });
            vm.toggleTextArea();
        };
